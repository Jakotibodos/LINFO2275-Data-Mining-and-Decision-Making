...
def markovDecision(layout,circle):
    ...
    # Next, I would say we create what happens when there is a trap (so the three types), and wemust be careful because there are two types of games (like circle and 'start when arriving at goal') and there is a trick at position 3. If I understand correctly, the goal is to define the transitions probabilities for each state and each dice? like dice 1 : all the transitions probabilities for each state, dice 2 : all the transition probabilities for each state, etc...